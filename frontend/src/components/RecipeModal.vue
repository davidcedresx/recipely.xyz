...
import { Ingredients, Recipes, Usages } from '../api'
    ...
    setup(props, context) {
        const state = reactive({
            recipe: {
                ...
                ingredients: props.recipe.usages?.map(usage => usage.ingredient) ?? [],
            },
            ingredients: {},
            loading: false,
            delete: false,
        ...
        async function onCreate() {
            ...
            const { _id } = await Recipes.create({ name: state.recipe.name })

            // register the usages
            await Promise.all(state.recipe.ingredients.map(ingredient => {
                Usages.create({ ingredient, recipe: _id })
            }))
        }

        async function onEdit() {
            const { _id } = await Recipes.update(props.recipe._id, {
            ...
            <section class="modal-card-body">
                <form @submit.prevent="submit">
                    ...
                    <div class="field">
                        ...
                        <div class="control">
                            <p v-if="Object.keys(state.ingredients) === 0">
